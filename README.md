...
![CI](https://github.com/idzingirai/cognito-local/workflows/CI/badge.svg)
...
### via Docker
    ...
    docker run --publish 9229:9229 idzingirai/cognito-local:latest
...
To persist your database between runs, mount the `/app/.cognito` volume to your host machine:
    ...
    docker run --publish 9229:9229 --volume $(pwd)/.cognito:/app/.cognito idzingirai/cognito-local:latest
...
`docker run -p4000:9229 idzingirai/cognito-local`
...
`docker run -p4000:4000 -e PORT=4000 idzingirai/cognito-local`
